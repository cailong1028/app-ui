...
util.setCurrentPath = function (vm, name) {
  ...
  vm.$store.state.appStore.routers.forEach(item => {
  ...
  if (name == 'home_index') {
    currentPathArr = [
      {
        title: util.handleTitle(vm, util.getRouterObjByName(vm.$store.state.appStore.routers, 'home_index')),
        path: '',
        name: 'home_index'
      }
    ];
    ...
    let currentPathObj = vm.$store.state.appStore.routers.filter(item => {
      if (item.children.length <= 1) {
...
util.openNewPage = function (vm, name, argu, query) {
  ...
  let pageOpenedList = vm.$store.state.appStore.pageOpenedList;
  ...
  if (!tagHasOpened) {
    let tag = vm.$store.state.appStore.tagsList.filter((item) => {
      if (item.children) {
...
util.toDefaultPage = function (routes, name, router, next) {
  console.log(`toDefaultPage: name: ${name}, next: ${next}`, name, router, next )
...
util.initRouter = function (vm) {
  ...
  if (!vm.$store.state.appStore.added) {
    // 第一次加载 读取数据
    ...
    // 加载菜单
    axios.get("/app/tPermission/getAll", { headers: { 'accessToken': accessToken } }).then(res => {
...
util.initMenuData = function (vm, data) {
  ...
  let tagsList = [];
  vm.$store.state.appStore.routers.map((item) => {
    if (item.children.length <= 1) {
...
util.initRouterNode = function (routers, data) {
  ...
  for (var item of data) {
    ...
    meta.title = menu.title ? menu.title + " - 版权家-会员项目管理平台" : null;
