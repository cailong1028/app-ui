...
export default {
  ...
  methods: {
    getCaptchaImg() {
      ...
      this.getRequest('/base/captcha/init', {}).then(res => {
        this.loadingCaptcha = false;
        if (res.success) {
          this.captchaId = res.result;
          this.captchaImg = '/app/base/captcha/draw/' + this.captchaId;
        }
      });
    },
    sendSmsCode() {
    ...
    afterLogin(res) {
      ...
      // 获取用户信息
      this.getRequest('/authUser/info').then(res => {
        if (res.success) {
          // 避免超过大小限制
          ...
          if (this.saveLogin) {
            // 保存7天
            cookie.set("userInfo", JSON.stringify(res.result), {
              expires: 7
            });
          } else {
            cookie.set("userInfo", JSON.stringify(res.result));
