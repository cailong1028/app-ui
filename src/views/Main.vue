    ...
    >
      ...
      >
        <div slot="top" class="logo-con">
          <img v-show="!shrink" src="../assets/logo.png" key="max-logo" style="width:200px;height:44px;"/>
          <img v-show="shrink" src="../assets/logo-min.png" key="min-logo"  style="height:44px;"/>
        </div>
      </shrinkable-menu>
    </div>
...
<script>
  ...
  import { /*ws, getMessageSendData, getOtherSet*/ api } from "@/api/index";
  export default {
    components: {
      shrinkableMenu,
      tagsPageOpened,
    ...
    data() {
      return {
        ...
        userId: "",
        isFullScreen: false,
        openedSubmenuArr: this.$store.state.appStore.openedSubmenuArr,
        firstThreeNav: [],
        lastNav: [],
        navType: 1
      };
    ...
    computed: {
      ...
      navList() {
        return this.$store.state.appStore.navList;
      },
      currNav() {
        return this.$store.state.appStore.currNav;
      },
      currNavTitle() {
        return this.$store.state.appStore.currNavTitle;
      },
      menuList() {
        return this.$store.state.appStore.menuList;
      },
      pageTagsList() {
        return this.$store.state.appStore.pageOpenedList; // 打开的页面的页面对象
      },
      currentPath() {
        return this.$store.state.appStore.currentPath; // 当前面包屑数组
      },
      avatarPath() {
        return localStorage.avatorImgPath;
      },
      cachePage() {
        return this.$store.state.appStore.cachePage;
      },
      lang() {
        return this.$store.state.appStore.lang;
      },
      menuTheme() {
        return this.$store.state.appStore.menuTheme;
      },
      mesCount() {
        return this.$store.state.appStore.messageCount;
      }
    },
    stompClient: {
